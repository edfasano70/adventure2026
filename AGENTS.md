...
- World editor: `python3 editor.py` — **based on PyQt5** (not pygame; needs PyQt5 installed). The game itself is still pygame; only the editor is Qt.
...
- The game's user config is `~/.config/adventure2026/config.xml` (XML, auto-written at runtime; tracks `fullscreen`/`volume`). The editor persists its own state in `config/adventure2/editor.xml` (XML).
